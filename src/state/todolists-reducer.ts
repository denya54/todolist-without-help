...
export const todolistsReducer = (state: TodolistsStateType, action: ActionsType) => {
    switch (action.type) {
        case "REMOVE-TODO":
...
export const addTodolistAC = (newTodoID: string, newTodoTitle: string) => {
    return {type: 'ADD-TODO', newTodoID, newTodoTitle} as const
}
